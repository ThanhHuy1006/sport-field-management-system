...
function getFieldImage(field: any) {
  const images = field.images || field.field_images || field.fieldImages || []

  if (Array.isArray(images) && images.length > 0) {
    const primaryImage = images.find((image: any) => image.is_primary || image.isPrimary)
...
function getDistrictFromAddress(address?: string | null) {
  if (!address) return "TP.HCM"

  const parts = address.split(",").map((part) => part.trim())
  const district = parts.find((part) => {
    const lowerPart = part.toLowerCase()
    return lowerPart.includes("quận") || lowerPart.includes("huyện") || lowerPart.includes("thủ đức")
  })

  return district || parts[parts.length - 2] || "TP.HCM"
}

function mapApiFieldToFeaturedField(field: any): FeaturedField {
  return {
    id: field.id || field.field_id || field.fieldId,
    name: field.name || field.field_name || field.fieldName || "Sân thể thao",
    image: getFieldImage(field),
    district: field.district || field.area || getDistrictFromAddress(field.address || field.location),
    price:
      Number(
        field.price ||
    ...
    available:
      ...
      field.is_available ??
      field.isAvailable ??
      (field.status ? field.status === "ACTIVE" || field.status === "approved" : true),
  }
}
...
export default function Home() {
  ...
  const [location, setLocation] = useState("")
  const [currentSlide, setCurrentSlide] = useState(0)
  const [featuredFields, setFeaturedFields] = useState<FeaturedField[]>([])

  useEffect(() => {
    const fetchFeaturedFields = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/fields?page=1&limit=8&sort=rating`, {
          method: "GET",
          cache: "no-store",
        })
        ...
        if (!res.ok) {
          throw new Error(json?.message || "Không thể tải danh sách sân nổi bật")
        }

        const rawFields = Array.isArray(json?.data)
          ? json.data
        ...
        setFeaturedFields(rawFields.map(mapApiFieldToFeaturedField))
      } catch (error) {
        console.error("Fetch featured fields failed:", error)
        setFeaturedFields([])
      }
    }
