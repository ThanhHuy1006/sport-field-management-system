// User vào /browse
// → BrowsePage render
// → lấy query ban đầu từ URL bằng useSearchParams
// → set state ban đầu: search, district, sport, sort, page
// → useEffect chạy
// → fetchFields()
// → getBrowseFields(query)
// → backend trả items + pagination
// → setFields(items)
// → setPagination(pagination)
// → render danh sách FieldCard
...
// User đổi filter
// → handleDistrictChange / handleSportChange / handlePriceChange
// → reset page 1
// → useEffect gọi API lại
...
// User đổi grid/list
// → setViewMode
// → chỉ đổi giao diện, không gọi API
"use client"
...
//tạo danh sách số trang để phân trang
...
// selectedDistrict  → quận/khu vực
// selectedSport     → môn thể thao
// selectedPrice     → khoảng giá
// sortBy            → kiểu sắp xếp
// viewMode          → grid hoặc list
// currentPage       → trang hiện tại
// fields      → danh sách sân nhận từ backend
...
export default function BrowsePage() {
  ...
  //đọc querry từ Url 
  //vi du: /browse?q=pickleball&district=Quận 7&sport=Cầu lông&page=2
  //searchParams.get("q")
  ...
  return (
    <>
      ...
      <section className="py-8">
        <div className="max-w-7xl mx-auto px-4">
          {isLoading ? (
            <div data-cy="fields-loading" className="text-center py-16">
              <p className="text-xl text-muted-foreground">Đang tải danh sách sân...</p>
            </div>
          ) : error ? (
            <div data-cy="fields-error" className="text-center py-16">
            ...
            <div data-cy="fields-empty" className="text-center py-16">
            ...
            <>
              {viewMode === "grid" ? (
                <div data-cy="fields-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {fields.map((field) => (
                    <div data-cy="field-card" key={field.id}>
                      <FieldCard field={field} viewMode="grid" />
                    </div>
                  ))}
                </div>
              ) : (
                <div data-cy="fields-list" className="flex flex-col gap-6">
                  {fields.map((field) => (
                    <div data-cy="field-card" key={field.id}>
                      <FieldCard field={field} viewMode="list" />
                    </div>
                  ))}
                </div>
              )}
              ...
              {pagination.totalPages > 1 && (
                <div data-cy="pagination" className="flex items-center justify-center gap-2 mt-8">
                  <Button
              ...
              <p data-cy="fields-result-count" className="text-center text-sm text-muted-foreground mt-4">
