...
} from "@/features/reviews/services/get-owner-reviews"
import { replyOwnerReview } from "@/features/reviews/services/reply-owner-review"
...
function getReviewUi(review: OwnerReviewItem) {
  return {
    id: review.id,
    fieldName: review.field?.field_name ?? `Sân #${review.field_id}`,
    author: review.user?.name ?? `Người dùng #${review.user_id}`,
    avatar: review.user?.avatar_url ?? "/placeholder.svg",
    rating: Number(review.rating || 0),
    text: review.comment ?? "Khách hàng không để lại bình luận.",
    date: formatDate(review.created_at),
    replied: Boolean(review.reply_text),
    reply: review.reply_text ?? "",
    replyDate: formatDate(review.reply_at),
  }
}
...
export default function OwnerReviewsPage() {
  ...
  const [currentPage, setCurrentPage] = useState(1)
  const [showReplyDialog, setShowReplyDialog] = useState(false)
  const [selectedReview, setSelectedReview] = useState<OwnerReviewItem | null>(null)
  const [reply, setReply] = useState("")
  const [filterField, setFilterField] = useState("all")
  const [filterStatus, setFilterStatus] = useState("all")
  ...
  const fieldOptions = Array.from(
    new Set(reviews.map((review) => getReviewUi(review).fieldName)),
  )

  const filteredReviews = reviews.filter((review) => {
    const reviewUi = getReviewUi(review)

    if (filterField !== "all" && reviewUi.fieldName !== filterField) return false
    if (filterStatus === "replied" && !reviewUi.replied) return false
    if (filterStatus === "unreplied" && reviewUi.replied) return false
    return true
  })
  ...
  const unrepliedCount = reviews.filter((review) => !review.reply_text).length
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      ...
      const response = await replyOwnerReview(selectedReview.id, {
        reply_text: reply.trim(),
      })

      setReviews((prev) =>
        prev.map((item) => (item.id === selectedReview.id ? response.data : item)),
      )
  ...
  const selectedReviewUi = selectedReview ? getReviewUi(selectedReview) : null
    ...
    <main className="min-h-screen bg-background">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="space-y-4 mb-8">
          {isLoading && (
            <Card className="p-12 text-center">
              <p className="text-muted-foreground text-lg">Đang tải đánh giá...</p>
            </Card>
          )}
          ...
          {!isLoading &&
            paginatedReviews.map((review) => {
              ...
              return (
                <Card key={reviewUi.id} className="p-6">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-start gap-3">
                      <img
                        src={reviewUi.avatar || "/placeholder.svg"}
                        alt={reviewUi.author}
                        className="w-12 h-12 rounded-full object-cover"
                      />
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-bold text-foreground">{reviewUi.author}</h4>
                          {!reviewUi.replied && <Badge variant="destructive">Chưa phản hồi</Badge>}
                        </div>
                        <p className="text-sm text-muted-foreground">{reviewUi.fieldName}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex items-center gap-1">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`w-4 h-4 ${
                                  i < reviewUi.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"
                                }`}
                              />
                            ))}
                          </div>
                          <span className="text-sm text-muted-foreground">{reviewUi.date}</span>
                        </div>
                      </div>
                    </div>
                  </div>

                  <p className="text-foreground mb-4">{reviewUi.text}</p>

                  {reviewUi.replied ? (
                    <div className="mt-4 p-4 bg-muted rounded-lg">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge>Phản hồi của bạn</Badge>
                        <span className="text-sm text-muted-foreground">{reviewUi.replyDate}</span>
                      </div>
                      <p className="text-foreground">{reviewUi.reply}</p>
                    </div>
                  ) : (
                    <Button
                      variant="outline"
      ...
      <Dialog open={showReplyDialog} onOpenChange={setShowReplyDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Phản hồi đánh giá</DialogTitle>
            <DialogDescription>Trả lời đánh giá từ {selectedReviewUi?.author}</DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-center gap-1 mb-2">
                {[...Array(5)].map((_, i) => (
                  <Star
                    key={i}
                    className={`w-4 h-4 ${
                      i < (selectedReviewUi?.rating || 0) ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground"
                    }`}
                  />
                ))}
              </div>
              <p className="text-sm text-foreground">{selectedReviewUi?.text}</p>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitReply} disabled={!reply.trim() || isSubmittingReply}>
