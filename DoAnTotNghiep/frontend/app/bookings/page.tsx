...
const getBookingStartTime = (timeRange: string) => {
  ...
  const confirmCancelBooking = async () => {
    ...
    try {
      ...
      await Promise.all([loadBookings(currentPage, activeTab), loadTabTotals()])
    } catch (error) {
      toast({
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (activeTab === "recurring") {
      return bookings.filter((booking) => booking.isRecurring && booking.status !== "CANCELLED")
    }
        ...
        <div className="space-y-4 mb-8">
          {isLoading ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium mb-2">Đang tải danh sách booking</h3>
              <p className="text-muted-foreground mb-4">Vui lòng chờ một chút...</p>
            </div>
          ...
          {selectedBooking && (
            <div className="flex flex-col items-center gap-6 py-4">
              <div className="p-4 bg-white rounded-xl shadow-lg">
                <QRCode value={qrToken || `BOOKING:${selectedBooking.bookingRef}`} size={200} />
              </div>
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setFieldQuality(star)}>
                      <Star
                        className={`w-5 h-5 ${
                          star <= fieldQuality ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                        }`}
                      />
                    </button>
                  ))}
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setService(star)}>
                      <Star
                        className={`w-5 h-5 ${
                          star <= service ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                        }`}
                      />
                    </button>
                  ))}
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setLocation(star)}>
                      <Star
                        className={`w-5 h-5 ${
                          star <= location ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                        }`}
                      />
                    </button>
                  ))}
                ...
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setFacilities(star)}>
                      <Star
                        className={`w-5 h-5 ${
                          star <= facilities ? "fill-yellow-400 text-yellow-400" : "text-gray-300"
                        }`}
                      />
                    </button>
                  ))}
